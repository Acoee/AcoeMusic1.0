    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                } else if (Math.abs(deltaX) > backView.getWidth()) { //��������ֻ��Ҫ���ڵײ�view�Ŀ�ȼ��ɻ����ɹ�
                    swap = true;
                    swapRight = deltaX > 0;
                }
...
//                }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (swipeMode == SwipeListView.SWIPE_MODE_NONE) {
                    ...
                    if (opened.get(downPosition)) {
                        if (swipeMode == SwipeListView.SWIPE_MODE_LEFT && deltaX2 < 0) {
                            deltaMode = 0;
                        } else if (swipeMode == SwipeListView.SWIPE_MODE_RIGHT && deltaX2 > 0) {
                            deltaMode = 0;
                        }
                    } else {
                        if (swipeMode == SwipeListView.SWIPE_MODE_LEFT && deltaX2 > 0) {
                ...
                if (deltaMode > slop && swipeCurrentAction == SwipeListView.SWIPE_ACTION_NONE && velocityY2 < velocityX2) {
                    swiping = true;
                    swipingRight = (deltaX2 > 0);
                    Log.d("SwipeListView", "deltaX: " + deltaX2 + " - swipingRight: " + swipingRight);
                    if (opened.get(downPosition)) {
                ...
                if (swiping && downPosition != ListView.INVALID_POSITION) {
                    if (opened.get(downPosition)) {
                        deltaX2 += openedRight.get(downPosition) ? viewWidth - rightOffset : -viewWidth + leftOffset;
                    }
                    move(deltaX2);
                    return true;
                }
