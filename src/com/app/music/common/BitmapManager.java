...
import com.app.music.common.cache.ImageFileCache;
import com.app.music.common.cache.ImageGetFromHttp;
import com.app.music.common.cache.ImageMemoryCache;
import com.app.music.view.ZoomImageView;
...
public class BitmapManager {  
    ...
    public BitmapManager(Context context){
    	...
    	this.defaultBmp = ImageUtils.readBitmapByResId(context, R.drawable.load_default_icon);
    	this.context = context;
    	this.memoryCache = ImageMemoryCache.getInstance(context);
    	this.fileCache = ImageFileCache.getInstance();
    }
     ...
     * @param imageView 要显示的view
     */
    public void loadBitmap(String url, ImageView imageView, int maxWidth) {  
    	if(maxWidth == 0){
    		loadBitmap(url, imageView);
    		return;
    	}
    	if(SystemUtils.isEmpty(url)){
			imageView.setImageBitmap(ImageUtils.zoomBitmap(defaultBmp, maxWidth));
			return;
		}
        imageViews.put(imageView, url);  
        //从内存缓存中获取
        ...
        if (bitmap != null) {  
			//显示缓存图片
            imageView.setImageBitmap(ImageUtils.zoomBitmap(bitmap, maxWidth));  
            imageViews.remove(imageView);
        } else {  
        	//加载SD卡中的图片缓存
        	bitmap = this.fileCache.getImage(url);
    		if(bitmap != null){
				...
				//线程加载网络图片
        		imageView.setImageBitmap(ImageUtils.zoomBitmap(defaultBmp, maxWidth));
        		queueJob(url, imageView, maxWidth);
        	}
        }  
    }  
    ...
    public void loadBackgroundBitmap(String url, View imageView, boolean isScrolling) {  
    	Log.i("mytreat", "isScrolling-------------------------->" + isScrolling);
    	if(SystemUtils.isEmpty(url)){
			imageView.setBackgroundDrawable(new BitmapDrawable(defaultBmp));
			return;
		}
    ...
    public void queueJob(final String url, final ZoomImageView zoomImage, final ProgressBar progress) {  
            ...
            public void handleMessage(Message msg) {  
                ...
                if (tag != null && tag.equals(url)) {
                    ...
                    if (msg.obj != null) {  
                    	...
                    	Log.i("queueJob", "zoom over 1");
                        //将图片缓存到文件和内存
                    	fileCache.saveBitmap(result, url);
    					memoryCache.addBitmapToCache(url, result);
                    } else {
                    	zoomImage.setImageBitmap(defaultBmp);
                    	Log.i("queueJob", "zoom over 2");
                    }
                }  
            }  
        };  
        ...
        pool.execute(new Runnable() {   
            public void run() {  
                ...
                message.obj = ImageGetFromHttp.getNetBitmap(context, url);
    ...
    public void queueJob(final String url, final ImageView imageView, final int maxWidth) {  
        /* Create handler in UI thread. */  
        final Handler handler = new Handler() {  
            public void handleMessage(Message msg) {  
                String tag = imageViews.get(imageView);  
                if (tag != null && tag.equals(url)) { 
                    ...
                    if (msg.obj != null) {  
                    	Bitmap result = (Bitmap) msg.obj;
                        imageView.setImageBitmap(ImageUtils.zoomBitmap(result, maxWidth));  
                        //将图片缓存到文件和内存
                    	fileCache.saveBitmap(result, url);
    					memoryCache.addBitmapToCache(url, result);
                    }
