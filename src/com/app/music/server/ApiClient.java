...
public class ApiClient {
	...
	public static InputStream doGet(AppContext appContext, String url) throws AppException {
		...
		do {
			try {
				...
				Log.i("acoe", "statusCode: " + statusCode);
	...
	public static InputStream doPost( String url, HashMap<String, Object> params, HashMap<String, File> files) throws AppException {
		...
		do {
			try {
				Log.i("acoe", "request server num-------->" + time);
				//获取httpClient对象
				if (url.startsWith("http://music.qq.com")) {
					httpClient = getHttpClient("GB2312");
				} else {
					httpClient = getHttpClient();
				}
				//获取Post方式的Method对象
				...
				Log.i("acoe", "HttpStatusCode-------->" + statusCode);
				if (statusCode != HttpStatus.SC_OK) {
					//throw AppException.http(statusCode);
					break;
				} 
